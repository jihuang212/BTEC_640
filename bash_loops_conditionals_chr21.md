...
echo $0
ls
cd Documents/btec_640/class_exercises
mkdir -p chromosome21
...
Extract info only for chromosome 21
Extract protein-coding genes
Lists gene and accessions
...
With grep -c, it would count how many lines match "chr21" and display the number of rows. Without it, it outputs all 52142 rows of chromosome 21 data and saves it into chr21.gtf
...
Gene name and accession numbers are in column 9
...
```
Count again how many genes you have now, and compare with the non-filtered file. -> 21108 with "wc -l refseq_chr21.gtf" to check
...
Use awk to split the text by looking for quotation marks because that's where the gene name and accession number is.
